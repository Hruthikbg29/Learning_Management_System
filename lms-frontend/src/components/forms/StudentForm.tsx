...
import type { StudentRequest } from "../../services/student.service";
...
interface Props {
  defaultValues?: Partial<StudentRequest>;
  onSubmit: (data: StudentRequest) => Promise<void>;
  onCancel: () => void;
  loading?: boolean;
}

const StudentForm = ({ defaultValues, onSubmit, onCancel, loading }: Props) => {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div>
        ...
        <input {...register("name")} className={fieldClass} placeholder="e.g. Aarav Patel" />
        {errors.name && <p className={errClass}>{errors.name.message}</p>}
      </div>
      <div>
        <label className={labelClass}>Email</label>
        <input {...register("email")} type="email" className={fieldClass} placeholder="student@email.com" />
